...
class GiftManageFragment :
    ...
    private val model: GiftManageViewModel by viewModels()
    ...
    override fun initData() {
        ...
        binding.refreshView.apply {
            setViewModel(model)
            setLifecycleOwner(this@GiftManageFragment)
            setAdapter(mGoodsManageAdapter)
        }
        model.apply {
            getGoodsClassify()
            goodsClassifyBean.observe(this@GiftManageFragment) {
                mClassifyManageAdapter.isFirst = false
                mClassifyManageAdapter.setList(it)
                if (it.isNotEmpty()) {
                    model.goodsClassifyId = it[0].id
                }
                this@GiftManageFragment.goodsClassifyBean = it
                binding.refreshView.loaData()
            }
        }
    }
