    ...
    try {
        if (input != null) {
            for (curChar in input) {
                output += if (curChar.toString().matches(Regex("[\\u4E00-\\u9FA5]+"))) {
                    val temp = PinyinHelper.toHanyuPinyinStringArray(curChar, format)
                    temp[0]
                } else curChar.toString()
            }
        }
...
private fun discountTheChinese(theStr: String): List<Map<String, Int>> {
    // 去除重复拼音后的拼音列表
    val mapList: MutableList<Map<String, Int>> = ArrayList()
    // 用于处理每个字的多音字，去掉重复
...
private fun parseTheChineseByObject(
    ...
    for (i in list.indices) {
        // 每一组集合与上一次组合的Map
        val temp: MutableMap<String, Int> = Hashtable()
        // 第一次循环，first为空
        if (first != null) {
            // 取出上次组合与此次集合的字符，并保存
            for (s in first.keys) {
...
class PinyinComparator : Comparator<MemberManageBean> {
    override fun compare(o1: MemberManageBean, o2: MemberManageBean): Int {
        return if (o1.letters == "@") {
            1
        } else if (o2.letters == "@") {
            -1
        } else {
